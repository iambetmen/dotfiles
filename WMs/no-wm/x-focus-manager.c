...
/*
 * some clients failt to reset focus on exit
 * we watch for that, and fix it
 *
 * instead of this, we also could use Elliott Hughe's clock
 * to launch x-alt-tab
 */
...
void set_atoms(Display *dpy) {
    int root = DefaultRootWindow(dpy);
    Atom wm_change_state = XInternAtom(dpy,"__NET_WM_STATE",False);
